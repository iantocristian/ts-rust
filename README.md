...
The compiler does not exist yet. What exists is the plan, upstream measurements, a tracking scaffold (a Cargo `xtask`, a file ledger, a function inventory, ADRs and generated status reports) and the two Phase 0 contract leaves: `ts_jsstring` and `ts_arena`. Mapped functions and implementation labels are reported separately from verified parity.
...
| [Harnesses](docs/harnesses.md) | What each experiment producer actually asserts, and what it leaves for a later sprint. |
...
| `crates/` | Compiler crates. `ts_jsstring` (source bytes, JavaScript strings, positions) and `ts_arena` (ids, owners, lazy storage, bundles) are the Phase 0 contract leaves; `ts_e3_harness` and `ts_e4_harness` answer the experiment probes. |
| `oracle/` | Go programs built against the pinned submodule that answer "what does Corsa do?" for the harnesses. |
