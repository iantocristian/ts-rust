...
def main():
    ...
    with oracle_export() as (checkout, env, pin):
        shutil.copyfile(ROOT / "scripts/s06_oracle/utilities_front_test.go",
                        checkout / "tsc/internal/ast/s06_utilities_front_test.go")
        observed = checkout / "utilities-front.tsv"
        env["S06_UTILITIES_FRONT_OUTPUT"] = str(observed)
        ...
        command(["go", "test", "-trimpath", "-mod=readonly", repo_path, "./internal/ast", "-run",
                 "^TestS06UtilitiesFront$", "-count=1"], cwd=checkout / "tsc", env=env)
        data = b"# pin " + pin.encode() + b"\n" + observed.read_bytes()
    target = ROOT / "data/s06/utilities-front-observations.tsv"
    if args.check:
        if target.read_bytes() != data:
            raise ValueError("pinned AST front utility observations changed")
    else:
