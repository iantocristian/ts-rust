...
every generated probe (including panic occurrence), then aggregates only the five
implemented criteria over the frozen scenario IDs. E3 covers arena contracts only.
"""
...
def text_probes(upstream):
    ...
    def add(group, criterion, op, text=b"", a=0, b=0, flag=False):
        counts[group] += 1
        probes.append({"id": f"{group}/{counts[group]:05}", "group": group, "criterion": criterion,
...
def compare(probes, oracle, rust):
    ...
    for label, results in (("oracle", oracle), ("rust", rust)):
        ...
        for result in results:
            ...
            if result["panic"] and result["value"] is not None:
