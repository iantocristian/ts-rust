...
func parseRequest(line []byte) (request, error) {
	...
	if err = fields(m, "version id primary op source_hex filename path script_kind jsx force operations"); err != nil {
		return request{}, err
	}
	...
	if m["primary"] != nil {
		if value, err := text(m["primary"]); err != nil || value == "" {
			return request{}, fmt.Errorf("invalid primary")
		}
	}
	...
	want := []string{"parse", "parsed_graph", "bind", "bound_graph", "repeat_bind", "repeated_graph"}
	if !ok || len(ops) != len(want) {
		return request{}, fmt.Errorf("invalid operation sequence")
	}
	for i, value := range want {
		if ops[i] != value {
			return request{}, fmt.Errorf("invalid operation order")
		}
	}
...
func main() {
	if err := run(os.Stdin, os.Stdout); err != nil {
		fmt.Fprintln(os.Stderr, "S07 oracle protocol:", err)
		os.Exit(2)
	}
}
