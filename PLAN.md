...
The [tracking specification](docs/TRACKING.md), [current status](STATUS.md), [dashboard](docs/status.html) and [ADR index](docs/adr/README.md) accompany this plan. S04 implements the `ts_jsstring` and `ts_arena` leaves; the scanner, parser and semantic checker remain pending. The text crate maps 47 upstream functions. Its local E4 run matched all 75,997 probes in 400 scenarios; seven ownership scenarios pass debug, release, Miri and AddressSanitizer checks with captured execution evidence. Owner/allocation counters measure tracked storage units, not global allocator calls or heap bytes. Full E3/E4 acceptance and the remaining experiments stay open. [S04](docs/S04.md) records the measured scope and reproduction commands.

The canonical `upstream/` submodule is registered at `1f70213d4922b434345f639b441681e470c7cfc1`; its actual Go oracle build and `--version` smoke have passing execution evidence. ADRs 0006, 0007 and 0013 and their design notes are accepted. The status workflow reruns leaf and bootstrap producers and requires S01/S04 on macOS and Linux; no remote workflow run is recorded here. Current evidence and prerequisite checks determine sprint closure under the [Phase 0 implementation plan](sprints/README.md). Leaf results do not establish full compiler parity.
...
1. Extend S04's implemented text and ownership leaves through the accepted design notes behind ADRs 0006, 0007 and 0013. Keep their measured helper, position, lazy-publication and disposal fixtures, and add retained checker results, synthetic nodes, builder emit-table dependencies and retirement versus result commitment as those production paths become available. The remaining E3/E4 integration gates stay explicit.
...
Upstream inventory numbers were measured on commit `1f70213d49` of microsoft/TypeScript (5 September 2026), excluding `testdata`, `node_modules` and test files unless stated. The package dependency order was produced by `go list` with Go 1.27.1 and is stored in `data/`. Draft 3.2 retains the independent repository, four native targets and owner-approved divergences. It makes file-owned lazy storage, mapper bundles, checker-local merges and shared-pool panic retirement explicit; adds source-byte and semantic id validation; separates prototype dependencies from full parity gates; keeps the pinned schema extractors authoritative; and requires full WebAssembly and Rust embedding acceptance before cut-over. S04's leaf measurements, updated 6 September 2026, are recorded separately in [docs/S04.md](docs/S04.md); the complete experiments and cut-over gates remain open.
