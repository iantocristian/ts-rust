...
- **Verified by.** E3 includes concurrent first-use JSDoc/token requests through shared snapshots, a mapper producing multiple supplemental files, release of owners in different orders, and stale/reused-id rejection. Evidence: `ast/ast.go:resolveJSDoc`, `GetOrCreateToken`, `contentmapper/transform.go`, and `project/parsecache.go:ContentMappedParseCache`.
...
| E3 Ownership | Shared files and snapshots, independent checker merges, concurrent lazy JSDoc/tokens, multi-output mapper bundles, builder caches, wrong-owner/stale/recycled ids, and a panic through one of two snapshots sharing a pool | Correct surviving queries; every invalid lookup rejected; every dependent registry observes retirement; owner/allocation counters return to the pre-scenario level after the final owner/lease drops; no Miri/AddressSanitizer violations | Measured in the ownership harness; repeated through real servers in 5/6. Sanitizers supplement explicit identity assertions; RSS is reported, not required to fall immediately after each drop |
...
3. Build the oracle from the unmodified pin. In a separate tooling worktree, run the pinned resolvers/extractors through the carried adapters, emit Rust, and compare regenerated TypeScript against the untouched client. Implement the Phase 0 dependency slices on top of `jsstring` and `arena`.
