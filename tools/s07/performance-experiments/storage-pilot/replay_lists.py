#!/usr/bin/env python3
"""Package/replay recorded list results without rerunning a native child."""
import argparse
import gzip
import io
import json
...
ROOT = probe.ROOT
BUILD_SHA = "14805a05dfe79ff38ea32d10aba0a2030119e60ff3f9fdadb9c57c329ed09dfc"
CAPTURE_SHA = "d9ba830ed1cea663e78988f3ddc28c69027e6c969c1508417d0ab1a64d705eda"
...
def replay_inputs():
    return {path.name: probe.runner.digest(path) for path in (Path(__file__), HERE / "test_replay_lists.py")}


def validate_members(members, build, capture):
    expected = {"build/manifest.json", "capture/manifest.json"}
    omitted = {record["path"] for record in build["artifacts"].values()}
    probe.require(len(omitted) == 2, "distinct binaries must be the only omitted build artifacts")
    for prefix, manifest, excluded in (("build", build, omitted), ("capture", capture, {"input.ndjson"})):
        for name, recorded in manifest["inventory"].items():
            if name in excluded:
                continue
            member = prefix + "/" + name
            expected.add(member)
            probe.require(member in members and identity(members[member]) == recorded, "recorded archive member changed: " + member)
    probe.require(set(members) == expected, "missing/extra archived source, helper or child output")
    return omitted
...
def read_members(path):
    ...
    with tarfile.open(path, "r:xz") as archive:
        for member in archive.getmembers():
            ...
            probe.require(member.isfile() and not name.is_absolute() and ".." not in name.parts and str(name) == member.name and member.name not in result, "invalid/duplicate archive member")
...
def package(build_dir, capture_dir, output):
    ...
    manifest = {"version": 1, "diagnostic_only": True, "kind": "s07_bis_list_distribution_archive", "scope": probe.SCOPE,
...
def replay(directory):
    ...
    probe.require(type(manifest["version"]) is int and manifest["version"] == 1 and manifest["diagnostic_only"] is True and manifest["kind"] == "s07_bis_list_distribution_archive" and manifest["scope"] == probe.SCOPE, "invalid list archive identity")
    ...
    with tempfile.TemporaryDirectory(prefix="s07-list-replay-") as temporary:
...
def main():
    ...
    parser.add_argument("--directory", type=Path, default=HERE / "results/2026-09-08")
    parser.add_argument("--build", type=Path, default=ROOT / "target/s07-bis/list-pilot-build-v2")
    parser.add_argument("--capture", type=Path, default=ROOT / "target/s07-bis/list-pilot-capture")
