#!/usr/bin/env python3
"""Package/replay recorded access results without rerunning a native child."""
import argparse
import gzip
import io
import json
...
probe = run_access.backend

HERE = Path(__file__).resolve().parent
ROOT = probe.ROOT
BUILD_SHA = "1ac32e32fa3c2f4ff44e2dbe53bcc020f46dc0773c1c7c31ad3dff62709ec89b"
CAPTURE_SHA = "940222dee95061632f272b92353774b0bce5b94a5a5b983e6b0d77f4c533e055"
...
def replay_inputs():
    ...
    return {path.name: probe.runner.digest(path) for path in (Path(__file__), HERE / "test_replay_access.py", HERE / "replay_lists.py")}
...
def package(build_dir, capture_dir, output):
    ...
    manifest = {"version": 1, "diagnostic_only": True, "kind": "s07_bis_list_access_archive", "scope": probe.SCOPE,
...
def replay(directory):
    ...
    probe.require(type(manifest["version"]) is int and manifest["version"] == 1 and manifest["diagnostic_only"] is True and manifest["kind"] == "s07_bis_list_access_archive" and manifest["scope"] == probe.SCOPE, "invalid access archive identity")
    ...
    with tempfile.TemporaryDirectory(prefix="s07-access-replay-") as temporary:
...
def main():
    ...
    parser.add_argument("--directory", type=Path, default=HERE / "results/2026-09-08-access")
    parser.add_argument("--build", type=Path, default=ROOT / "target/s07-bis/access-pilot-build")
    parser.add_argument("--capture", type=Path, default=ROOT / "target/s07-bis/access-pilot-capture")
