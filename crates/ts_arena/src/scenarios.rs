...
pub fn stale_and_recycled_ids_rejected() -> Measurement {
    let mut observation = Observation::new();
    {
        ...
        let retained = owner.node(old_id).unwrap();
        drop(owner);
...
pub fn concurrent_lazy_storage() -> Measurement {
    let mut observation = Observation::new();
    {
        ...
        let initial = owner
            ...
            .unwrap();
        let retained = initial.node(0).unwrap();
    ...
    for first in 0..4 {
        for second in 0..4 {
            for third in 0..4 {
                ...
                assert_eq!(ids.len(), 3);
                let retained = handles[3].as_ref().unwrap().node(node_ids[2]).unwrap();
...
pub fn owners_return_to_baseline() -> Measurement {
    let mut observation = Observation::new();
    {
        ...
        let mut scope = Scope::new();
        scope.insert(file);
        let node = scope.import(id).unwrap();
        let symbol = scope.import_symbol(symbol_id).unwrap();
        observation.observe();
        drop(scope);
