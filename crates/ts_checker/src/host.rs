...
//! | `GetSourceFileForResolvedModule`, `GetResolvedModule`, `GetEmitModuleFormatOfFile`, `GetImpliedNodeFormatForEmit`, `GetEmitSyntaxForUsageLocation`, `GetRedirectForResolution`, `SourceFileMayBeEmitted` | In the trait; the loader has the inputs, the lookups are P2 work |
//! | `BindSourceFiles` | Satisfied by construction: program files are `CompletedFile`s |
//! | `GetSourceFileMetaData` | P2: the type lives in `ts_compiler` today and must move below the checker (upstream keeps it in `ast`) |
...
//! | `GetProjectReferenceFromOutputDts`, `GetProjectReferenceFromSource`, `GetRedirectTargets`, `GetSourceOfProjectReferenceIfOutputIncluded` | Explicitly unsupported: project references are excluded from the denominator; the implementation must say so, not return `None` |
...
pub trait CheckerHost {
    ...
    fn get_emit_module_format_of_file(&self, file_name: &[u8]) -> ModuleKind;
    fn get_emit_syntax_for_usage_location(
        &self,
        file_name: &[u8],
        usage_location: NodeId,
    ) -> ResolutionMode;
    fn get_implied_node_format_for_emit(&self, file_name: &[u8]) -> ModuleKind;
    ...
    fn source_file_may_be_emitted(&self, file: &CompletedFile, force_dts_emit: bool) -> bool;
    fn is_source_file_default_library(&self, path: &[u8]) -> bool;
    fn get_redirect_for_resolution(&self, file_name: &[u8]) -> Option<&ParsedCommandLine>;
