...
//! Node IDs are first assigned by comparisons while building the inverse index.
//! Consequently the pinned SortFunc comparison order is part of that operation's
//! observable behavior; an otherwise equivalent Rust sort is insufficient.
use std::cmp::Ordering;

type Compare<'a, T> = &'a mut dyn FnMut(&T, &T) -> Ordering;

pub(crate) fn sort<T>(data: &mut [T], compare: &mut impl FnMut(&T, &T) -> Ordering) {
    let length = data.len();
